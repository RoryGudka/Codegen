...
export async function createAssistant(
  ...
  try {
    ...
    console.info("Assistant ID:", output.id);
    console.info(`Most relevant files:\n${files.map(([p]) => p).join("\n")}`);
    console.info("Output:", `.codegen/outputs/output-${output.id}.txt`);
    console.info(`Output: http://localhost:5000/${output.id}`);
